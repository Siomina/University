...
@WebServlet("/schedule/dayLecturerSchedule")
public class DayLecturerScheduleServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private static final Logger log = LogManager.getLogger(DayLecturerScheduleServlet.class.getName());
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			scheduleUnits = scheduleUnitService.findDayLecturerSchedule(lecturerId, calendar);
			request.setAttribute("scheduleUnits", scheduleUnits);
			request.getRequestDispatcher("/listDayLecturerSchedule.jsp").forward(request, response);
