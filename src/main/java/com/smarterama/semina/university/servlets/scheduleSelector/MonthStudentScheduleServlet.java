...
@WebServlet("/schedule/monthStudentSchedule")
public class MonthStudentScheduleServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private static final Logger log = LogManager.getLogger(MonthStudentScheduleServlet.class.getName());
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			scheduleUnits = scheduleUnitService.findMonthStudentSchedule(studentId, calendar);
			request.setAttribute("scheduleUnits", scheduleUnits);
			request.getRequestDispatcher("/listMonthStudentSchedule.jsp").forward(request, response);
